...
# Piper: a local TTS (Text-to-Speech) engine — synthesizes speech offline
    ...
    def speak(self, text: str) -> None:
        ...
        try:
            subprocess.run(
                ...
                input=text,
                text=True, 
                check=True, # Piper exits with a non-zero status code, Python raises an exception
                capture_output=True,
            )
            # read the synthesized WAV back as int16 PCM samples
            with wave.open(wav_path, "rb") as wf:
                sr = wf.getframerate()  # sr = sample rate of the synthesized audio
...
# Edge: Microsoft's cloud TTS — used as a fallback when Piper isn't available
...
# match a sentence boundary: terminal punctuation followed by whitespace
_SENTENCE_END = re.compile(r"(?<=[.!?])\s+")


# sentence splitter: enables streaming TTS — speak each sentence as the LLM emits it
class SentenceSplitter:
    """Feed token-deltas in; get back complete sentences as soon as they form."""

    def __init__(self) -> None:
        # buffer holds text not yet emitted as a complete sentence
        self._buf = ""

    # chunk = a token (small text fragment) streamed from the LLM
    def feed(self, chunk: str) -> list[str]:
        ...
        if len(parts) <= 1:
            return []
        # everything but the last part is complete; keep the tail buffered
